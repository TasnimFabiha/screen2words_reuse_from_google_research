...
import create_tf_example_fn

FLAGS = flags.FLAGS

flags.DEFINE_string('task', 'CREATE_TF_EXAMPLE',
                    'Task name, could be CREATE_VOCAB or CREATE_TF_EXAMPLE.')
flags.DEFINE_string('dataset_paths', './result/',
                    'List of dataset paths, separated by comma.')

flags.DEFINE_string('json_file_path', './screen_summaries.json', 'json label file path.')
flags.DEFINE_string('word_vocab_path', './tmp_widget/word_vocab_from_create_vocab.txt', 'Word vocab file path.')
                    #'./word_embedding/glove.6B.300d.copy.txt', 'Word vocab file path.')
#flags.DEFINE_string('word_vocab_path', './tmp/word_vocab.txt-00000-of-00001', 'Word vocab file path.')
...
flags.DEFINE_string('output_tfexample_path', './tmp/tfexample.txt', 'Path to output tf.Example.')
...
# def _generate_screen_id_and_captions_pair_new(json_file_path):
#   """Generates pair of screen id and MTurk labels for each screen."""
#   labels = list()
#   with tf.io.gfile.GFile(json_file_path) as f:
...
#        summary = row['summary']
#        labels.append([screenId, summary])
#   return labels
...
class CreateTFExampleFn(beam.DoFn):
  ...
  def start_bundle(self):
    ...
    ## vocab is not used as I expected
...
def main(argv):
  ...
  runners.DirectRunner().run(pipeline)
