...
import screen2words_eval
import screen2words_experiment_config
import screen2words_keras_input as input_utils
...
def load_embed(file_name, vocab_size):
  ...
  with tf.compat.v1.gfile.Open(file_name, 'r') as embed_file:
    vocab = []
    embeds = []
    depth = -1
...
class ScreenCaptionModel(tf.keras.Model):
  ...
  def __init__(self, hparams):
    ...
    self._word_vocab = []
    with tf.compat.v1.gfile.Open(self._hparams['word_vocab_path']) as f:
