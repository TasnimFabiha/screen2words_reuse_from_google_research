...
def _truncate_and_pad_attention_box(labels, max_boxes):
  ...
  for l in labels:
    ...
    #changes by fabiha
    #--------
    attention_box = l[1][:max_boxes] if len(l)>1 else []
    #-----
    if to_xywh:
...
#fabiha--I addded image file image_path
def _get_features_from_all_nodes(all_nodes, image, image_path):
...
def create_tf_example(file_prefix, labels, tokenizer, word_vocab,
  ...
  with tf.io.gfile.GFile(image_path, 'rb') as f:
    ...
    features = _get_features_from_all_nodes(all_nodes, image, image_path)
